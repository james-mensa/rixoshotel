...
export default function SignUpPage() {
  ...
  const initialState = {
    ...
    comfirmPassword:{value: "", error: true },
  ...
  const handleSubmit =async () => {
    ...
    if (isValid) {
      ...
      setLoading(true);
     const res= await dispatch(AuthService.register(formData));
     if(res){
      setLoading(false)
     }
  ...
  return (
    ...
    <Label sx={styles.description}>
      ...
      <Box sx={styles.rightComponent}>
        <Label sx={styles.title}>Continue to register with your email</Label>
        <InputText
          name="email"
        ...
        />
          ...
          error={state.submissionAttempt && state.name.error}
        ...
        <InputText
          ...
          error={state.submissionAttempt && state.comfirmPassword.error}
          errorMessage={"Please enter your password again"}
          value={state.comfirmPassword.value}
          handleOnChange={handleChange("comfirmPassword")}
        />
        <AuthButton label="Sign up" onClick={handleSubmit}  loading={isLoading}/>
        <Stack
          sx={{ width: "100%" }}
          direction="row"
          alignItems={"center"}
